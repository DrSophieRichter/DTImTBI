...
```{r}
#Now select only one - the 2-3 week - biomarker sample per patient

#CENTER-TBI defined the 2-3 weeks timepoint as 10-27 days
bio <-
  bio %>%
  filter(Time_to_bio >= 10 & Time_to_bio <= 27)
...
#NFL at 2-3 weeks
num <- pat %>% filter(is.na(NFL)==TRUE) %>% nrow()
paste0("Patients missing NFL data at 2-3 weeks: ", num)
pat <- pat %>% filter(is.na(NFL)==FALSE)
paste0("Patients with NFL sampled at 2-3 weeks: ", nrow(pat))
...
maxit = 1 #number of iterations during multiple imputation - for this data convergence observed after 10
