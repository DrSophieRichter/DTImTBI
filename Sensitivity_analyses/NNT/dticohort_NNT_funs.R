...
format_tab <- function(tab, mysource, myvar){
  ...
  #Calculate NNS 
  #(number needed to scan to identify 1 patient with incomplete recovery)
  scanned <-
    mysource %>%
    filter(mysource[, myvar]  > as.numeric(tab[["cutoff", "est"]])) %>%
    nrow()
  TP <- 
    mysource %>%
    filter(mysource[, myvar]  > as.numeric(tab[["cutoff", "est"]]) & Recovery == "incomplete") %>%
    nrow()
  NNS <- scanned/TP
  NNS <- format(round(NNS,1), nsmall = 1)
