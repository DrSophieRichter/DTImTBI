...
imp_list[[i]] <- adjust_for_age(mydata = imp_list[[i]])
...
saveRDS(orig_pat_list, "RDS/orig_pat_list_dticohort.rds")
...
###########There is a  problem with the ASA variable
for (i in 1:length(orig_pat_list)){
  orig_pat_list[[i]]$ASA <- 
...
saveRDS(orig_pat_list, "RDS/orig_pat_list.rds")
...
saveRDS(imp_boot_data_list, "RDS/imp_boot_data_list.rds")
```


```{r}
#Plot DTI score stratified by outcome
ggplot(data = imp_boot_data_list[[2]][[1]], 
         aes(x = Recovery, 
             y = DTI_score, 
             group = Recovery)) +
  geom_boxplot() +
