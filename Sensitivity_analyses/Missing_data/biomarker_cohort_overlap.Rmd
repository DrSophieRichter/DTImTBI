...
```{r}
#########Load imputed datasets from original analysis
orig_pat_list <- readRDS("/Users/sophierichter/Documents/Medicine/PhD/DTI_mTBI/RDS_bio/orig_pat_list_GFAP.rds")
```

```{r}
#######Select only patients from overlap cohort
...
for (i in 1:length(orig_pat_list)){
  orig_pat_list[[i]] <- 
    orig_pat_list[[i]] %>%
    filter(subjectId %in% overlap$subjectId)
}

###########There is a  problem with the GCS variable
# Only 1 patient has GCS 13 --> some bootstrap samples will not have any GCS 13 so cannot test
# Models with this "new level"
# So I will add the one person with GCS 13 to the GCS 14 group

for (i in 1:length(orig_pat_list)){
  orig_pat_list[[i]]$GCSScoreBaselineDerived <- 
    ifelse(orig_pat_list[[i]]$GCSScoreBaselineDerived %in% c("13", "14"), "<15" ,"15") 
  orig_pat_list[[i]]$GCSScoreBaselineDerived <- 
    factor(orig_pat_list[[i]]$GCSScoreBaselineDerived) 
}

#In variable Education only 4 or 5 patients have th lowest educational level
# so I need to combine those
for (i in 1:length(orig_pat_list)){
  orig_pat_list[[i]]$Education <- 
    ifelse(orig_pat_list[[i]]$Education %in% c("below GCSE", "GCSE/AS/A levels equivalent"), "below diploma" ,as.character(orig_pat_list[[i]]$Education)) 
  orig_pat_list[[i]]$Education <- 
    factor(orig_pat_list[[i]]$Education) 
}

#Similar problem with variable ASA
for (i in 1:length(orig_pat_list)){
  orig_pat_list[[i]]$ASA <- 
    ifelse(orig_pat_list[[i]]$ASA %in% c("mild disease", "severe or life-threatening disease"), "not healthy" ,"healthy") 
  orig_pat_list[[i]]$ASA <- 
    factor(orig_pat_list[[i]]$ASA) 
}


saveRDS(orig_pat_list, "RDS_bio/orig_pat_list.rds")
```
